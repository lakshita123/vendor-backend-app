...
function drawSectionTitle(doc, title, subtitle, accent) {
  ...
  const y = doc.y;
  const stripeColor = accent || COLORS.info;

  doc.save().roundedRect(MARGIN, y, 4, subtitle ? 28 : 18, 2).fill(stripeColor).restore();
  doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(11.5).text(title, MARGIN + 12, y, {
    lineBreak: false,
  });

  if (subtitle) {
    doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8).text(subtitle, MARGIN + 12, y + 15, {
...
function drawHeader(doc, submission, validation) {
  ...
  doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7.5).text("VENDOR REVIEW REPORT", x + 18, y + 13, {
    lineBreak: false,
  });
  doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(18).text("Submission Quality Summary", x + 18, y + 25, {
    lineBreak: false,
  });
  ...
  doc.fillColor(sc.text).font("Helvetica-Bold").fontSize(10).text(statusLabel(validation.status), x + w - 140, y + 30, {
    width: 122,
    align: "center",
    lineBreak: false,
  });
...
function drawSummaryCards(doc, validation) {
  ...
  cards.forEach((card, i) => {
    ...
    doc.fillColor(card.text).font("Helvetica-Bold").fontSize(7.5).text(card.label.toUpperCase(), x + 8, y + 9, {
      width: cardW - 16,
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(19).text(String(card.value), x + 8, y + 26, {
      width: cardW - 16,
      lineBreak: false,
    });
  });
...
function drawKeyValueGrid(doc, entries) {
  ...
  for (let i = 0; i < printable.length; i += 2) {
    ...
    row.forEach((entry, ci) => {
      ...
      drawRoundedCard(doc, { x, y, w: colW, h: rowH, fill: COLORS.surface, stroke: COLORS.border, r: 8 });
      doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7.5).text(String(entry.label).toUpperCase(), x + 10, y + 8, {
        width: colW - 20,
        lineBreak: false,
      });
      const val = safeText(entry.value);
      const truncated = val.length > 80 ? `${val.slice(0, 77)}...` : val;
      doc.fillColor(COLORS.ink).font("Helvetica").fontSize(9).text(truncated, x + 10, y + 21, {
        width: colW - 20,
        lineBreak: false,
      });
    });
...
function drawDocumentCards(doc, documents) {
  ...
  (documents || []).forEach((document, index) => {
    ...
    // Skip address parts sub-objects and null values
    const SKIP_KEYS = new Set(["addressParts", "officialAddressParts", "textSample"]);
    ...
    const cardH = Math.max(50 + rowCount * 26, document.extractionError ? 86 : 76);
    ...
    doc.fillColor(COLORS.muted).font("Helvetica").fontSize(7.5).text(safeText(document.originalname), x + 28, y + 23, {
      width: cw - 140,
      lineBreak: false,
    });
    ...
    if (!rows.length) {
      ...
      rows.forEach((row, ri) => {
        ...
        const rx = x + 12 + col * colW;
        const ry = y + 40 + line * 26;
        const val = safeText(row.value);

        doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7).text(row.label.toUpperCase(), rx, ry, {
          width: colW - 4,
          lineBreak: false,
        });
        doc.fillColor(COLORS.ink).font("Helvetica").fontSize(8.5).text(val.length > 60 ? `${val.slice(0, 57)}...` : val, rx, ry + 10, {
          width: colW - 4,
          lineBreak: false,
        });
      });
    ...
    if (document.extractionError) {
      doc.fillColor(COLORS.warning).font("Helvetica").fontSize(8).text(`Note: ${safeText(document.extractionError)}`, x + 12, y + cardH - 14, {
        width: cw - 24,
        lineBreak: false,
      });
    }
...
function drawValidationCards(doc, checks) {
  ...
  if (!regular.length) {
    doc.fillColor(COLORS.muted).font("Helvetica").fontSize(9).text("No cross-document checks were run.", MARGIN, doc.y, {
      lineBreak: false,
    });
    doc.y += 14;
    return;
  ...
  regular.forEach((check) => {
    ...
    doc.fillColor(colors.text).font("Helvetica-Bold").fontSize(7.5).text(check.passed ? "PASS" : "REVIEW", x + 10, y + 16, {
      width: 52,
      align: "center",
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(9).text(check.title, x + 70, y + 12, {
      width: cw - 80,
      lineBreak: false,
    });

    const col2X = x + 14 + colW + 8;
    doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7).text("EXPECTED", x + 14, y + 34, {
      width: colW,
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica").fontSize(8.5).text(safeText(check.expected), x + 14, y + 44, {
      width: colW - 4,
      lineBreak: true,
    });
    doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7).text("ACTUAL", col2X, y + 34, {
      width: colW,
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica").fontSize(8.5).text(safeText(check.actual), col2X, y + 44, {
      width: colW - 4,
      lineBreak: true,
    });
...
function drawFaceVerificationSection(doc, validation) {
  ...
  if (!faceResults.length) {
    ...
    doc.fillColor(COLORS.muted).font("Helvetica").fontSize(9).text("No geo tag photo uploaded. Face verification was not performed.", MARGIN + 12, y + 12, {
      lineBreak: false,
    });
    doc.y = y + 44;
    return;
  ...
  faceResults.forEach((result) => {
    if (result.error) {
      ...
      doc.fillColor("#9a6700").font("Helvetica-Bold").fontSize(7.5).text("SKIPPED", MARGIN + 10, y + 16, {
        width: 60,
        align: "center",
        lineBreak: false,
      });
      doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(9.5).text(`Geo Tag vs ${result.label}`, MARGIN + 80, y + 12, {
        lineBreak: false,
      });
      doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8.5).text(String(result.error).slice(0, 120), MARGIN + 80, y + 26, {
        width: cw - 90,
        lineBreak: false,
      });
      doc.y = y + 60;
    ...
    const colors = result.match
      ? { fill: COLORS.tealSoft, badge: "#e6faf8", stroke: "#99e6df", text: COLORS.teal }
      : { fill: COLORS.dangerSoft, badge: "#feecec", stroke: COLORS.dangerMid, text: COLORS.danger };
    ...
    doc.fillColor(colors.text).font("Helvetica-Bold").fontSize(8.5).text(result.match ? "MATCH" : "NO MATCH", x + 12, y + 15, {
      width: 80,
      align: "center",
      lineBreak: false,
    });

    doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(11).text(`Geo Tag vs ${result.label} Card`, x + 104, y + 11, {
      lineBreak: false,
    });
    doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8).text("128-D facial descriptor comparison", x + 104, y + 24, {
      lineBreak: false,
    });
    ...
    doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(8.5).text("Similarity Confidence", barX, barY - 11, {
      lineBreak: false,
    });
    doc.fillColor(barColor).font("Helvetica-Bold").fontSize(15).text(`${pct}%`, x + cw - 54, y + 38, {
      width: 40,
      align: "right",
      lineBreak: false,
    });
    ...
    details.forEach((d, ci) => {
      ...
      doc.fillColor(COLORS.info).font("Helvetica-Bold").fontSize(7).text(d.label.toUpperCase(), dx, detailY, {
        width: colW - 4,
        lineBreak: false,
      });
      doc.fillColor(COLORS.ink).font("Helvetica").fontSize(8.5).text(safeText(d.value), dx, detailY + 10, {
        width: colW - 4,
        lineBreak: false,
      });
    });
...
function drawIssueCards(doc, issues) {
  ...
  if (!issues.length) {
    ...
    doc.fillColor(COLORS.success).font("Helvetica-Bold").fontSize(9.5).text("No issues detected. This submission looks healthy.", MARGIN + 14, y + 13, {
      lineBreak: false,
    });
    doc.y = y + 46;
    return;
  ...
  ordered.forEach((issue, index) => {
    ...
    doc.fillColor(colors.text).font("Helvetica-Bold").fontSize(7.5).text(issue.severity.toUpperCase(), x + 10, y + 16, {
      width: 62,
      align: "center",
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(9.5).text(`${index + 1}. ${issue.title}`, x + 82, y + 12, {
      width: cw - 92,
      lineBreak: false,
    });
    doc.fillColor(COLORS.ink).font("Helvetica").fontSize(8.5).text(detailText, x + 12, y + 34, {
      width: cw - 26,
      lineBreak: true,
    });
...
function drawAccuracyMeter(doc, validation) {
  ...
  doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(10.5).text("Report Accuracy Meter", x + 14, y + 10, {
    lineBreak: false,
  });
  doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8).text("Reliability estimate based on readable files, passed checks, face results, and open issues.", x + 14, y + 25, {
    width: cw - 130,
    lineBreak: false,
  });
  ...
  doc.fillColor(COLORS.ink).font("Helvetica-Bold").fontSize(20).text(`${score}%`, x + cw - 96, y + 10, {
    width: 82,
    align: "right",
    lineBreak: false,
  });
  doc.fillColor(COLORS.muted).font("Helvetica").fontSize(8.5).text(label, x + cw - 96, y + 36, {
    width: 82,
    align: "right",
    lineBreak: false,
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    drawKeyValueGrid(doc, [
      ...
      { label: "Geo Address", value: submission.geoAddress },
      {
        ...
        value: submission.geoLatitude && submission.geoLongitude ? `${submission.geoLatitude}, ${submission.geoLongitude}` : null,
      },
    ...
    drawDocumentCards(doc, validation.extractedDocuments || []);
    drawDivider(doc);
